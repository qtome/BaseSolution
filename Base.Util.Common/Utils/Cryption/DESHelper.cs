...
namespace Base.Util.Common.Utils.Cryption
{
    ...
    public class DESHelper
    {
        ...
        public static string Encrypt(string text, string shareKey)
        {
            ...
            des.Key = Encoding.ASCII.GetBytes(HashHelper.MD5Encrypt(shareKey, MD5Length.L32).Substring(0, 8));
            des.IV = Encoding.ASCII.GetBytes(HashHelper.MD5Encrypt(shareKey, MD5Length.L16).Substring(0, 8));// 向量
            using (MemoryStream memStream = new MemoryStream())
            {
                CryptoStream crypStream = new CryptoStream(memStream, des.CreateEncryptor(), CryptoStreamMode.Write);
        ...
        public static string Decrypt(string text, string shareKey)
        {
            ...
            using (MemoryStream memStream = new MemoryStream())
            {
                CryptoStream crypStream = new CryptoStream(memStream, des.CreateDecryptor(), CryptoStreamMode.Write);
        ...
        {
            return TripleEncrypt(text, defaultKey);
        }
        ...
        /// <param name="shareKey">共享密钥</param> 
        /// <returns>密文</returns> 
        public static string TripleEncrypt(string text, string shareKey)
        {
            TripleDESCryptoServiceProvider des = new TripleDESCryptoServiceProvider();
            des.Key = HashHelper.GetMD5(shareKey);
        ...
        {
            return TripleDecrypt(text, defaultKey);
        }
        ...
        /// <param name="shareKey">共享密钥</param> 
        /// <returns></returns> 
        public static string TripleDecrypt(string text, string shareKey)
        {
            TripleDESCryptoServiceProvider des = new TripleDESCryptoServiceProvider();
            des.Key = HashHelper.GetMD5(shareKey);
            des.Mode = CipherMode.ECB;
        ...
        /// <param name="text">待验证明文</param>
        /// <returns>是否一致</returns>
        public static bool VerifyTripleCryption(string encode, string text)
        {
            if (string.IsNullOrWhiteSpace(encode)) throw new ArgumentNullException(nameof(encode));
            if (string.IsNullOrWhiteSpace(text)) throw new ArgumentNullException(nameof(text));
            string result = TripleDecrypt(encode);
            return result.Equals(text);
        }
        ...
        /// <param name="shareKey">共享密钥</param>
        /// <returns>是否一致</returns>
        public static bool VerifyTripleCryption(string encode, string text, string shareKey)
        {
            if (string.IsNullOrWhiteSpace(encode)) throw new ArgumentNullException(nameof(encode));
            if (string.IsNullOrWhiteSpace(text)) throw new ArgumentNullException(nameof(text));
            if (string.IsNullOrWhiteSpace(shareKey)) throw new ArgumentNullException(nameof(shareKey));
            string result = TripleDecrypt(encode, shareKey);
            return result.Equals(text);
        }
