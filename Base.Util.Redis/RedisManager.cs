...
namespace Base.Util.Redis
{
    ...
    public class RedisManager : IDisposable
    {
        ...
        public string SysCustomKey { get; }

        #region 构造函数

        public RedisManager(RedisConfig redisConfig, int dbNum = 0)
        {
            DbNum = dbNum;
            SysCustomKey = redisConfig.SysCustomKey;
        ...
        /// <param name="key"></param>
        /// <param name="value"></param>
        public void ListLeftPushRange<T>(string key, List<T> value)
        {
            key = AddSysCustomKey(key);
            value.ForEach(f =>
            {
                Do(db => db.ListLeftPush(key, ConvertJson(f)));
            });
        }
        ...
        public List<string> GetAllCacheKeys()
        {
            try
            {
                ...
                foreach (var endpoint in endpoints)
                {
                    ...
                    result.AddRange(server.Keys(DbNum).Select(x => RemoveSysCustomKey(x.ToString())));
                }
        ...
        private string RemoveSysCustomKey(string key)
        {
            var prefixKey = SysCustomKey;
            return key.Replace(prefixKey, "");
        }
        ...
        {
            string result = value is string ? value.ToString() : JsonHelper.SerializeObject(value);
            return result;
        }
        ...
        private T ConvertObj<T>(RedisValue value)
        {
            ...
            {
                T t = JsonHelper.DeserializeObject<T>(value);
                if (t == null)
                {
